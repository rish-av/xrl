...
class D4RLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        next_actions_seq = self.actions[idx + 1:idx + self.seq_len + 1]  

        return states_seq, actions_seq, next_states_seq, next_actions_seq
...
def train(model, dataloader, num_epochs=10, learning_rate=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        for states, actions, next_states in dataloader:
            optimizer.zero_grad()
            predicted_states, predicted_actions, quantization_loss = model(states, actions, next_states)
            state_loss = mse_loss_fn(predicted_states, next_states)
            action_loss = mse_loss_fn(predicted_actions, actions)
...
model = TrajectoryTransformerVQ(state_dim, action_dim, hidden_dim, num_heads, num_layers, max_len, num_embeddings)
